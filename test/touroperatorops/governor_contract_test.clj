(ns touroperatorops.governor-contract-test
...
(deftest tour-record-full-flow
  (testing "clean tour-record proposal -> auto-commit at phase 3"
    (let [db (store/seed-db)
          actor (op/build db)
          ctx {:actor-id "test-1" :phase 3}
          result (exec-request actor "t1"
                               {:op :log-tour-record :tour-id "tour-1" :patch {:participant "Nakamura party" :check-in "2026-07-14"}}
                               ctx)]
      (is (some? result))
...
(deftest traveler-safety-concern-always-escalates
  (testing ":flag-traveler-safety-concern escalates for human approval, regardless of phase/confidence"
    (let [db (store/seed-db)
          actor (op/build db)
          ctx {:actor-id "test-2" :phase 3}
          result (exec-request actor "t2"
                               {:op :flag-traveler-safety-concern :tour-id "tour-1"
                                :patch {:concern "altitude sickness" :confidence 0.99}}
                               ctx)]
      (is (some? result))
...
(deftest high-cost-vendor-settlement-escalates-then-commits-on-approval
  (testing "an over-threshold vendor-settlement proposal escalates, then commits once a human approves"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2b"
                               {:op :coordinate-vendor-settlement :tour-id "tour-1"
                                :patch {:vendor "bulk-charter-transfer-co" :estimated-amount 2500}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
...
(deftest unregistered-tour-hard-hold
  (testing "unregistered tour -> permanent HARD hold, never escalates"
    (let [db (store/seed-db)
          actor (op/build db)
          ctx {:actor-id "test-3" :phase 3}]
      (exec-request actor "t3"
                     {:op :log-tour-record :tour-id "unknown-tour"
                      :patch {:participant "unknown"}}
                     ctx)
      (is (= 0 (count (store/coordination-log db)))
          "HARD hold must never commit"))))

(deftest unverified-tour-hard-hold
  (testing "registered but unverified tour -> permanent HARD hold"
    (let [db (store/seed-db)
          actor (op/build db)
          ctx {:actor-id "test-4" :phase 3}
          result (exec-request actor "t4"
                               {:op :log-tour-record :tour-id "tour-3"
                                :patch {:participant "unknown"}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
          "unverified tour must HARD hold"))))
  ...
  (testing "proposal with :effect :commit (not :propose) -> hard hold"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t5"
                               {:op :log-tour-record :tour-id "tour-1"
                                :patch {:participant "Nakamura party"}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
...
(deftest scope-excluded-content-hard-hold
  (testing "proposal drifting into traveler-safety-clearance-finalization scope -> permanent hard hold"
    (let [db (store/seed-db)
          actor (op/build db)
  ...
  (testing "every decision leaves immutable audit facts"
    (let [db (store/seed-db)
      ...
      (exec-request actor "t8a"
                     {:op :log-tour-record :tour-id "tour-1" :patch {:participant "Nakamura party"}}
                     ctx)
      (exec-request actor "t8b"
                     {:op :log-tour-record :tour-id "unknown" :patch {:participant "Nakamura party"}}
                     ctx)
  ...
  (testing "an approver rejecting an escalated proposal results in HOLD, never commit"
    (let [db (store/seed-db)
      ...
      (exec-request actor "t9"
                     {:op :flag-traveler-safety-concern :tour-id "tour-1"
                      :patch {:concern "unattended baggage"}}
                     ctx)
