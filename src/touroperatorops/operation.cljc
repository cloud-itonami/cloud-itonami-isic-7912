(ns touroperatorops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (TourOperatorOpsAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the TourOperatorGovernor (:govern)
  ...
  human operator/tour-operations manager. The approver resumes with
...
(defn build
  ...
  `touroperatorops.store/Store`).
  opts:
    :advisor      -- a `touroperatorops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; TourOperatorOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; TourOperatorGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :tour-id (:tour-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :tour-id (:tour-id request) :by (:by approval)}]}
            {:disposition :hold
